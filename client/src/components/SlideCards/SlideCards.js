import React from 'react';
import './SlideCards.css';

class SlideCards extends React.Component {
    render() {
        return(
            <div className="slidecard-wrap">

                <div className="row">
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1464054313797-e27fb58e90a9?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=996&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                    
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1458668383970-8ddd3927deed?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=1004&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                        
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1422393462206-207b0fbd8d6b?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=1000&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="row">
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1464054313797-e27fb58e90a9?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=996&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                    
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1458668383970-8ddd3927deed?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=1004&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                        
                    <div className="slidecard-tile"> 
                        <img src='https://images.unsplash.com/photo-1422393462206-207b0fbd8d6b?dpr=1&auto=format&crop=entropy&fit=crop&w=1500&h=1000&q=80'/>
                        <div className="slidecard-text">
                            <h1>Lorem ipsum.</h1>
                            <h2 className="slidecard-animate-text">More lorem ipsum bacon ipsum.</h2>
                            <p className="slidecard-animate-text">Bacon ipsum dolor amet pork belly tri-tip turducken, pancetta bresaola pork chicken meatloaf. Flank sirloin strip steak prosciutto kevin turducken. </p>
                            <div className="slidecard-dots">
                                <span></span>
                                <span></span>
                                <span></span>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        )
    }
}
export default SlideCards